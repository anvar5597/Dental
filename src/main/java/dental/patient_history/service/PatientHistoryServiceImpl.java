/**
 * Author: Anvar Olimov User:user Date:6/24/2024 Time:3:04 PM
 */
...
public class PatientHistoryServiceImpl implements PatientHistoryService {
  ...
  public String create(@NotNull PatientAddDto dto) {
    ...
    patientHistoryEntity.setSale(0);
  ...
  private static TServiceListDto getServiceListDto(@NotNull PatientHistoryEntity entity, int i) {
    ...
    tServiceListDto1.setTeethName(
        entity.getTeethServiceEntities().get(i).getTeethServiceKey().getTeeth().getTeethName());
    tServiceListDto1.setServiceName(
        entity.getTeethServiceEntities().get(i).getTeethServiceKey().getService().getServiceName());
    tServiceListDto1.setPrice(
        entity.getTeethServiceEntities().get(i).getTeethServiceKey().getService().getPrice());
  ...
  @Override
  public List<PatientResponseDto> findByEmpIdIsServicedBetween(Long id, LocalDateTime start,
      LocalDateTime end) {
    return repository.findByEmployeesId(id)
        .stream()
        .filter(entity -> entity.getIsServiced()
  ...
  @Override
  public List<PatientResponseDto> findPaidPatient() {
    return repository.findAll()
        .stream()
        .filter(PatientHistoryEntity::getActive)
        .filter(PatientHistoryEntity::getIsPaid)
        .filter(PatientHistoryEntity::getIsServiced)
        .map(this::toDto)
        .toList();
  }
  ...
  public List<MonthlyIncomeExpenseDTO> getMonthlyTotalIncomeAndExpense() {
    ...
    for (Object[] row : results) {
      ...
      Long totalSale = row[4] != null ? (Long) row[4] : 0L;

      list.add(
          new MonthlyIncomeExpenseDTO(year, month, null, totalIncome, totalExpense, totalSale));
    }
  ...
  public List<MonthlyIncomeExpenseDTO> getMonthlyIncomeAndExpensePerEmployee() {
    ...
    for (Object[] row : results) {
      ...
      Long totalSale = (row[5] != null) ? (Long) row[5] : 0L;  // chegirma

      MonthlyIncomeExpenseDTO dto = new MonthlyIncomeExpenseDTO(year, month, employeeId,
          totalIncome, totalExpense, totalSale);
      monthlyList.add(dto);
    }
  ...
  public PatientResponseDto toDto(PatientHistoryEntity entity) {
    ...
    responseDto.setTeethServiceEntities(getServiceListDto(entity));
    responseDto.setExpense(entity.getExpense());
    responseDto.setSale(entity.getSale());
    responseDto.setCreatedAt(entity.getCreatedAt());
